...
由于当前阿珍只有 GitHub Deploy Key，不能直接调用 GitHub Issues API，因此需要通过 GitHub Actions 中转：阿珍只负责把结构化 intake JSON push 到仓库，Actions 使用仓库自带的 `GITHUB_TOKEN` 创建或更新 Issue。
...
### 场景 B：相似反馈追加到已有 Issue

当反馈与已有 Issue 相似时，阿珍需要把新用户原文追加为 Issue 评论，而不是创建重复 Issue。
...
GitHub Actions 读取 `intake/issues/` 下的 JSON：
...
8. 处理失败时，文件进入 `intake/failed/`，并在日志中记录失败原因。
...
请阿强按 PRD Review 闸口审查本 PRD 初稿，重点检查：

- 是否只写 What、不写 How。
- 验收标准是否用户可感知、可验证。
- 非目标和风险是否清楚。
- 是否可以作为考试链路第二段 Review 输入物。
